...
const AshChat = () => {
  const { setShowInterrogation, userProfile } = useApp();
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
        setIsTyping(false);
        setMessages(prev => [...prev, { sender: 'ash', text: "I see. Let's go with something bold." }]);
    }, 1000);
  };
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.isAction ? (
              ...
              <div className={`max-w-[85%] p-3 text-sm leading-relaxed shadow-sm ${msg.sender === 'user' ? 'bg-black text-white rounded-2xl rounded-br-none' : 'bg-white text-gray-800 rounded-2xl rounded-bl-none border border-gray-100'}`}>{msg.text}</div>
            )}
          </div>
        ))}
