...
const ASH_LOGO = '/ash-logo.png';// src/pages/ChatPage.jsx
import { useState, useRef, useEffect, useCallback } from 'react';
import { Send, Plus, Trash2, Edit3, X, Copy, Check, RefreshCw, Sparkles, ChevronRight, BookOpen, Paperclip, Lock, Menu } from 'lucide-react';
...
const KEY   = 'ash_convos_v4';

const PRODUCTS = [
  { id:'hush-001', name:'Midnight Silk Dress',  price:'180', cat:'Dress',      img:'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=400&fit=crop', link:'https://timeforhush.com/products/midnight-silk-dress',    desc:'Black silk, open back.' },
  { id:'hush-002', name:'Executive Blazer',      price:'250', cat:'Outerwear',  img:'https://images.unsplash.com/photo-1548624149-f32144679638?w=400&fit=crop', link:'https://timeforhush.com/products/executive-blazer',       desc:'Oversized, sharp shoulders.' },
  { id:'hush-003', name:'Viper Leather Pants',   price:'300', cat:'Bottoms',    img:'https://images.unsplash.com/photo-1548624313-0396c75e4b1a?w=400&fit=crop', link:'https://timeforhush.com/products/viper-leather-pants',    desc:'High waisted, vegan leather.' },
  { id:'hush-004', name:'Raw Edge Denim',        price:'220', cat:'Denim',      img:'https://images.unsplash.com/photo-1542272604-787c3835535d?w=400&fit=crop', link:'https://timeforhush.com/products/raw-edge-denim',         desc:'Japanese selvage. Straight leg.' },
...
const CSS = `
  ...
  @keyframes vPulse { 0%{filter:brightness(1) drop-shadow(0 0 0 #C41E1E)} 40%{filter:brightness(1.5) drop-shadow(0 0 10px #C41E1E)} 100%{filter:brightness(1) drop-shadow(0 0 0 #C41E1E)} }
  ...
  .asl{animation:slideL .3s cubic-bezier(.16,1,.3,1) forwards}
  .asr{animation:slideR .3s cubic-bezier(.16,1,.3,1) forwards}
  .cur{display:inline-block;width:2px;height:.85em;background:#C41E1E;margin-left:1px;vertical-align:text-bottom;animation:blink 1s step-end infinite}
  .vp{animation:vPulse 1.8s ease-in-out infinite;transform-origin:center}
  .vt{animation:vFloat 1.8s ease-in-out infinite}
  .sc::-webkit-scrollbar{width:4px}
  .sc::-webkit-scrollbar-track{background:transparent}
  .sc::-webkit-scrollbar-thumb{border-radius:4px;background:#D5D0C8}
  .hh .hv{opacity:0;transition:opacity .15s}
  .hh:hover .hv{opacity:1}
  ...
  .ci{cursor:pointer;transition:background .15s}
  .ch{cursor:pointer;transition:all .15s} .ch:hover{transform:translateY(-1px)}
  .pc{transition:all .2s} .pc:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,0.1)}
`;
...
function useStream(){
  const[text,setText]=useState('');const[running,setRunning]=useState(false);const r=useRef(null);
  const stream=useCallback((full,done)=>{setText('');setRunning(true);let i=0;
    const t=()=>{i=Math.min(i+Math.floor(Math.random()*3)+1,full.length);setText(full.slice(0,i));
      if(i<full.length)r.current=setTimeout(t,11+Math.random()*9);else{setRunning(false);done?.();}};
    r.current=setTimeout(t,60);},[]);
  const stop=()=>{clearTimeout(r.current);setRunning(false)};
...
function MD({text}){
  ...
  while(i<lines.length){const l=lines[i];
    if(l.startsWith('**')&&l.endsWith('**')&&!l.slice(2,-2).includes('**')){
      ...
      const its=[];while(i<lines.length&&(lines[i].startsWith('- ')||lines[i].startsWith('* '))){its.push(lines[i].slice(2));i++;}
      out.push(<ul key={'u'+i} style={{listStyle:'none',padding:0,margin:'5px 0'}}>
        {its.map((it,j)=><li key={j} style={{display:'flex',gap:7,margin:'4px 0',fontFamily:sans,fontSize:14,lineHeight:1.7,color:C.txt}}>
          <span style={{color:C.acc,flexShrink:0,fontSize:10,marginTop:4}}>✦</span>
          <span dangerouslySetInnerHTML={{__html:fmt(it)}}/>
        </li>)}
      </ul>);continue;}
    else if(l.trim()==='---'){out.push(<hr key={i} style={{border:'none',borderTop:`1px solid ${C.bdr}`,margin:'10px 0'}}/>);}
    else if(l.trim()===''){if(i>0&&lines[i-1].trim())out.push(<div key={i} style={{height:4}}/>);}
    else{out.push(<p key={i} style={{margin:'2px 0',fontFamily:sans,fontSize:14,lineHeight:1.78,color:C.txt}} dangerouslySetInnerHTML={{__html:fmt(l)}}/>);}
    i++;}
  return <div>{out}</div>;
}
...
function Av({isAsh,pulse}){
  return isAsh
    ?<div style={{width:32,height:32,borderRadius:'50%',background:'#111',border:`1px solid ${C.bdr}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,overflow:'hidden'}}>
        <img src={ASH_LOGO} className={pulse?'vp':''} style={{width:20,height:20,objectFit:'contain'}} alt="Ash"/>
      </div>
...
function Thinking(){
  return <div className="aup" style={{display:'flex',gap:12,marginBottom:22,maxWidth:760,marginLeft:'auto',marginRight:'auto'}}>
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,background:C.surf,border:`1px solid ${C.bdr}`,borderRadius:'4px 16px 16px 16px',padding:'12px 18px',boxShadow:C.shd}}>
        <img src={ASH_LOGO} className="vp" style={{width:22,height:22,objectFit:'contain',flexShrink:0}} alt=""/>
        <span className="vt" style={{fontFamily:serif,fontStyle:'italic',fontSize:15,color:C.fnt,letterSpacing:'.04em'}}>Styling your look…</span>
      </div>
    </div>
  </div>;
}
...
function Lookbook({look,onClose}){
  ...
  return <>
    ...
    <div className="asr sc" style={{position:'fixed',top:0,right:0,bottom:0,width:296,background:C.sb,borderLeft:`1px solid ${C.bdr}`,zIndex:101,overflowY:'auto',display:'flex',flexDirection:'column'}}>
      <div style={{padding:'14px 16px',borderBottom:`1px solid ${C.bdr}`,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,background:C.sb}}>
        ...
        <button className="ib" onClick={onClose} style={{color:C.fnt,width:26,height:26,borderRadius:6}}><X size={14}/></button>
      </div>
...
function FitModal({onClose,onSave}){
  ...
  const Pill=({label,on,click})=><button onClick={click} style={{padding:'6px 12px',fontSize:11,fontWeight:600,fontFamily:sans,textTransform:'uppercase',letterSpacing:'.07em',borderRadius:24,border:`1.5px solid ${on?C.acc:C.bdr}`,background:on?C.acc:'transparent',color:on?'#fff':C.mid,cursor:'pointer',transition:'all .15s'}}>{label}</button>;
  return <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center',backdropFilter:'blur(4px)'}}>
    <div className="afade" style={{background:C.surf,width:'100%',maxWidth:480,borderRadius:'20px 20px 0 0',overflow:'hidden',boxShadow:'0 -16px 48px rgba(0,0,0,0.18)'}}>
      <div style={{background:'#111',color:'#fff',padding:'18px 22px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
          <div style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.14em',color:C.acc,marginBottom:3}}>🔒 FIT ID</div>
          <div style={{fontFamily:serif,fontWeight:600,fontSize:19}}>{['Who Are You?','Your Frame','Style DNA',"You're In ✦"][step]}</div>
        </div>
        <button onClick={onClose} style={{background:'rgba(255,255,255,.1)',border:'none',color:'#fff',width:30,height:30,borderRadius:'50%',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={14}/></button>
      </div>
      {step<3&&<div style={{display:'flex',gap:3,padding:'8px 22px',background:'#f5f5f2'}}>{[0,1,2].map(i=><div key={i} style={{flex:1,height:2,borderRadius:2,background:i<=step?C.acc:C.bdr,transition:'background .3s'}}/>)}</div>}
      <div className="sc" style={{padding:'18px 22px',maxHeight:'58vh',overflowY:'auto'}}>
        {step===0&&<>
          <p style={{fontFamily:sans,fontSize:13,color:C.mid,marginBottom:14,lineHeight:1.6}}>One-time setup. Ash remembers you.</p>
          <label style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.1em',color:C.fnt,display:'block',marginBottom:5}}>Your Name</label>
          <input value={form.name} onChange={e=>up('name',e.target.value)} placeholder="e.g. Sofia" style={{width:'100%',border:`1.5px solid ${C.bdr}`,borderRadius:10,padding:'10px 13px',fontSize:14,fontFamily:sans,color:C.txt,background:'#f9f9f7',outline:'none',boxSizing:'border-box'}}/>
          <button onClick={()=>setStep(1)} disabled={!form.name} style={{width:'100%',marginTop:14,background:form.name?'#111':'#ccc',color:'#fff',border:'none',borderRadius:10,padding:'12px',fontSize:13,fontWeight:600,fontFamily:sans,textTransform:'uppercase',letterSpacing:'.08em',cursor:form.name?'pointer':'default'}}>Continue →</button>
        </>}
        {step===1&&<>
          <label style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.1em',color:C.fnt,display:'block',marginBottom:5}}>Height (cm)</label>
          <input type="number" value={form.height} onChange={e=>up('height',e.target.value)} placeholder="e.g. 168" style={{width:'100%',border:`1.5px solid ${C.bdr}`,borderRadius:10,padding:'10px 13px',fontSize:14,fontFamily:sans,color:C.txt,background:'#f9f9f7',outline:'none',boxSizing:'border-box',marginBottom:14}}/>
          <label style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.1em',color:C.fnt,display:'block',marginBottom:8}}>Body Type</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:7,marginBottom:16}}>{BODIES.map(b=><Pill key={b} label={b} on={form.bodyType===b} click={()=>up('bodyType',b)}/>)}</div>
          <button onClick={()=>setStep(2)} style={{width:'100%',background:'#111',color:'#fff',border:'none',borderRadius:10,padding:'12px',fontSize:13,fontWeight:600,fontFamily:sans,textTransform:'uppercase',letterSpacing:'.08em',cursor:'pointer'}}>Continue →</button>
        </>}
        {step===2&&<>
          <label style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.1em',color:C.fnt,display:'block',marginBottom:8}}>Zodiac</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:14}}>{ZODIACS.map(z=><Pill key={z} label={z} on={form.zodiac===z} click={()=>up('zodiac',z)}/>)}</div>
          <label style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.1em',color:C.fnt,display:'block',marginBottom:8}}>Skin Tone</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:14}}>{SKINS.map(s=><Pill key={s} label={s} on={form.skinTone===s} click={()=>up('skinTone',s)}/>)}</div>
          <label style={{fontFamily:sans,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.1em',color:C.fnt,display:'block',marginBottom:8}}>Style DNA <span style={{fontWeight:400,letterSpacing:0,textTransform:'none',color:C.fnt}}>(pick all)</span></label>
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:16}}>{STYLEZ.map(s=><Pill key={s} label={s} on={form.styleDNA.includes(s)} click={()=>tog(s)}/>)}</div>
          <button onClick={()=>{onSave(form);setStep(3);}} style={{width:'100%',background:C.acc,color:'#fff',border:'none',borderRadius:10,padding:'12px',fontSize:13,fontWeight:600,fontFamily:sans,textTransform:'uppercase',letterSpacing:'.08em',cursor:'pointer'}}>✦ Activate FIT ID</button>
        </>}
        {step===3&&<div style={{textAlign:'center',padding:'20px 0'}}>
          <div style={{fontSize:48,marginBottom:14}}>✦</div>
          <div style={{fontFamily:serif,fontWeight:600,fontSize:22,color:C.txt,marginBottom:6}}>Welcome, {form.name.split(' ')[0]}.</div>
          <p style={{fontFamily:sans,fontSize:13,color:C.mid,lineHeight:1.7,marginBottom:20}}>Ash now knows exactly how to dress you.</p>
          <button onClick={onClose} style={{background:'#111',color:'#fff',border:'none',borderRadius:10,padding:'12px 28px',fontSize:13,fontWeight:600,fontFamily:sans,textTransform:'uppercase',letterSpacing:'.08em',cursor:'pointer'}}>Talk to Ash →</button>
        </div>}
      </div>
    </div>
  </div>;
...
function Msg({msg,onRegen,onLook,isLast,streamText,isStreaming}){
  const[copied,setCopied]=useState(false);
  const isAsh=msg.role==='assistant';
  const content=isStreaming?streamText:msg.content;
  const copy=()=>{navigator.clipboard?.writeText(content||'');setCopied(true);setTimeout(()=>setCopied(false),2000);};
  return <div className="aup hh" style={{display:'flex',gap:12,marginBottom:24,maxWidth:760,marginLeft:'auto',marginRight:'auto',flexDirection:isAsh?'row':'row-reverse'}}>
    ...
    <div style={{flex:1,minWidth:0}}>
      ...
      {isAsh&&msg.look&&!isStreaming&&<button onClick={()=>onLook(msg.look)} className="ch" style={{marginTop:10,display:'inline-flex',alignItems:'center',gap:6,background:C.accBg,border:`1px solid ${C.acc}40`,borderRadius:24,padding:'6px 13px',fontFamily:sans,fontSize:11,fontWeight:600,color:C.acc,textTransform:'uppercase',letterSpacing:'.08em',cursor:'pointer'}}><Sparkles size={11}/> View Look: {msg.look.title}</button>}
      {!isStreaming&&<div className="hv" style={{display:'flex',gap:3,marginTop:6}}>
        <button className="ib" onClick={copy} style={{width:26,height:26,borderRadius:6,color:copied?'#22c55e':C.fnt}}>{copied?<Check size={12}/>:<Copy size={12}/>}</button>
        {isAsh&&isLast&&<button className="ib" onClick={onRegen} style={{width:26,height:26,borderRadius:6,color:C.fnt}}><RefreshCw size={12}/></button>}
      </div>}
    </div>
  </div>;
}
...
function Sidebar({convos,active,onSel,onNew,onDel,onRen,onClose}){
  ...
  return <>
    ...
    <div className="asl sc" style={{position:'fixed',top:0,left:0,bottom:0,width:252,background:C.sb,borderRight:`1px solid ${C.bdr}`,zIndex:49,display:'flex',flexDirection:'column',boxShadow:'4px 0 20px rgba(0,0,0,0.08)'}}>
      <div style={{padding:'13px 14px',borderBottom:`1px solid ${C.bdr}`,display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:7}}>
          <div style={{width:22,height:22,borderRadius:'50%',background:'#111',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden'}}><img src={ASH_LOGO} style={{width:13,height:13,objectFit:'contain'}} alt=""/></div>
          <span style={{fontFamily:serif,fontWeight:600,fontSize:13,color:C.acc,letterSpacing:'.15em'}}>ASH ×</span>
        </div>
        <div style={{display:'flex',gap:5}}>
          <button className="ib" onClick={onNew} style={{background:C.accBg,border:`1px solid ${C.acc}40`,borderRadius:7,width:26,height:26,color:C.acc}}><Plus size={13}/></button>
          <button className="ib" onClick={onClose} style={{width:26,height:26,borderRadius:7,color:C.fnt}}><X size={13}/></button>
        </div>
      </div>
      ...
      <div className="sc" style={{flex:1,overflowY:'auto',padding:'2px 5px 8px'}}>
        ...
        {convos.map(c=><div key={c.id} className="ci hh" onClick={()=>{onSel(c.id);onClose();}} style={{borderRadius:9,padding:'8px 9px',marginBottom:2,background:c.id===active?C.accBg:'transparent',border:`1px solid ${c.id===active?C.acc+'40':'transparent'}`}}>
          {eid===c.id
            ...
            :<><div style={{fontFamily:sans,fontSize:13,fontWeight:500,color:C.txt,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',lineHeight:1.3}}>{c.title}</div>
              ...
              <div className="hv" style={{display:'flex',gap:2,marginTop:3}}>
                <button onClick={e=>{e.stopPropagation();setEid(c.id);setEtxt(c.title);}} className="ib" style={{width:18,height:18,borderRadius:3,color:C.fnt}}><Edit3 size={9}/></button>
                <button onClick={e=>{e.stopPropagation();onDel(c.id);}} className="ib" style={{width:18,height:18,borderRadius:3,color:C.fnt}}><Trash2 size={9}/></button>
              </div>
            </>}
        </div>)}
      </div>
...
export default function ChatPage(){
  ...
  // KEY FIX: measure available height and apply it so layout works correctly
  useEffect(()=>{
  ...
  const save=useCallback((m,id)=>{
    ...
    const first=m.find(x=>x.role==='user');if(!first)return sid;
  ...
  const regen=async()=>{const lu=[...msgs].reverse().find(m=>m.role==='user');if(!lu||loading)return;setMsgs(p=>{const l=p.filter(m=>m.role==='assistant').at(-1);return l?p.filter(m=>m.id!==l.id):p;});await send(lu.content);};
  ...
  return (
    // ref + measured height = input always stays at bottom
    <div ref={wrapRef} style={{display:'flex',flexDirection:'column',overflow:'hidden',background:C.bg,width:'100%'}}>
      <style>{CSS}</style>

      {/* ── SLIM BAR — history + new + collection + VIP ── */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'7px 16px',borderBottom:`1px solid ${C.bdr}`,background:C.surf,flexShrink:0,boxShadow:C.shd}}>
        <div style={{display:'flex',alignItems:'center',gap:7}}>
          <button className="ib" onClick={()=>setSbOpen(true)} title="History" style={{width:32,height:32,borderRadius:8,color:C.mid,background:C.bg,border:`1px solid ${C.bdr}`}}><Menu size={15}/></button>
          <button className="ib" onClick={newChat} title="New chat" style={{width:32,height:32,borderRadius:8,color:C.acc,background:C.accBg,border:`1px solid ${C.acc}40`}}><Plus size={15}/></button>
        </div>
        <button onClick={()=>setLookbook('all')} style={{display:'flex',alignItems:'center',gap:5,background:C.bg,border:`1px solid ${C.bdr}`,borderRadius:20,padding:'5px 13px',cursor:'pointer',fontFamily:sans,fontSize:11,fontWeight:500,color:C.mid}}>
          <BookOpen size={12}/> Collection
        </button>
        {profile
          ?<div style={{display:'flex',alignItems:'center',gap:5,background:C.accBg,border:`1px solid ${C.acc}40`,borderRadius:20,padding:'5px 12px',fontFamily:sans,fontSize:11,fontWeight:600,color:C.acc}}>✦ {profile.name.split(' ')[0]}</div>
          :<button onClick={()=>setFitOpen(true)} style={{display:'flex',alignItems:'center',gap:5,background:'#111',border:'none',borderRadius:20,padding:'6px 14px',cursor:'pointer',fontFamily:sans,fontSize:11,fontWeight:600,color:'#fff',letterSpacing:'.04em'}}><Lock size={11}/> Unlock VIP</button>
        }
      </div>

      {/* ── VIP BANNER — fixed under slim bar ── */}
      {!profile&&(
        <div style={{background:'#111',color:'#fff',padding:'8px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexShrink:0}}>
          <p style={{fontFamily:sans,fontSize:12,lineHeight:1.4,margin:0}}>🔒 <strong>Unlock your FIT ID</strong> — Ash styles better when she knows you.</p>
          <button onClick={()=>setFitOpen(true)} style={{fontFamily:sans,fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'.08em',background:C.acc,color:'#fff',border:'none',padding:'6px 13px',borderRadius:7,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>Get Started</button>
        </div>
      )}

      {/* ── MESSAGES — flex:1 + minHeight:0 = stays in its lane ── */}
      <div className="sc" style={{flex:1,minHeight:0,overflowY:'auto',padding:'24px 18px 8px'}}>
        {isWelcome&&(
          <div className="afade" style={{textAlign:'center',marginBottom:40}}>
            <div style={{width:68,height:68,borderRadius:'50%',background:'#111',border:`1px solid ${C.bdr}`,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px',boxShadow:'0 6px 24px rgba(0,0,0,0.15)',overflow:'hidden'}}>
              <img src={ASH_LOGO} style={{width:42,height:42,objectFit:'contain'}} alt="ASH"/>
            </div>
        ...
        {display.map((m,i)=>m.isPend?<Thinking key={m.id}/>:<Msg key={m.id} msg={m} onRegen={regen} onLook={setLookbook} isLast={i===lastAshI} streamText={stxt} isStreaming={m.id===streamId&&srun}/>)}
        <div ref={bottomRef}/>
      </div>
      ...
      {isWelcome&&(
        <div style={{padding:'0 18px 12px',display:'flex',flexWrap:'wrap',gap:7,maxWidth:760,margin:'0 auto',width:'100%',boxSizing:'border-box',flexShrink:0}}>
          {QUICK.map(q=><button key={q} onClick={()=>send(q)} className="ch" style={{background:C.surf,border:`1px solid ${C.bdr}`,borderRadius:24,padding:'7px 15px',fontFamily:sans,fontSize:12,fontWeight:500,color:C.mid,boxShadow:C.shd,cursor:'pointer'}}>{q}</button>)}
        </div>
      )}
      ...
      {imgPrev&&(
        <div style={{padding:'0 18px 7px',maxWidth:760,margin:'0 auto',width:'100%',boxSizing:'border-box',flexShrink:0}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:7,background:C.surf,border:`1px solid ${C.bdr}`,borderRadius:10,padding:7}}>
            ...
            <button className="ib" onClick={()=>setImgPrev(null)} style={{color:C.fnt,width:18,height:18}}><X size={12}/></button>
          </div>
        </div>
      )}
      ...
      <div style={{padding:'9px 16px 13px',borderTop:`1px solid ${C.bdr}`,background:C.surf,flexShrink:0}}>
        <div style={{maxWidth:760,margin:'0 auto'}}>
          <div style={{display:'flex',gap:7,alignItems:'center',background:C.bg,border:`1.5px solid ${C.bdr}`,borderRadius:20,padding:'9px 10px 9px 14px',boxShadow:C.shd}}>
            ...
            <button className="ib" onClick={()=>fileRef.current?.click()} style={{width:28,height:28,borderRadius:7,color:C.fnt,flexShrink:0}}><Paperclip size={14}/></button>
            <input
              value={input}
            ...
            />
            {loading
              ?<button className="ib" onClick={stop} style={{width:34,height:34,borderRadius:10,background:C.accBg,color:C.acc,flexShrink:0}}><X size={14}/></button>
              :<button className="ib" onClick={()=>send(input,imgPrev)} disabled={!input.trim()&&!imgPrev} style={{width:34,height:34,borderRadius:10,background:(input.trim()||imgPrev)?C.acc:C.bdr,color:(input.trim()||imgPrev)?'#fff':C.fnt,flexShrink:0}}><Send size={14}/></button>
            }
          </div>
      ...
      {fitOpen&&<FitModal onClose={()=>setFitOpen(false)} onSave={p=>{setProfile(p);setFitOpen(false);setMsgs(prev=>[...prev,{role:'assistant',content:`FIT ID activated. ${p.name.split(' ')[0]}, now I can actually dress you. What are we solving?`,id:String(Date.now())}]);}}/>}
    </div>
  );
}
const serif = "'Cormorant Garamond', Georgia, serif";
