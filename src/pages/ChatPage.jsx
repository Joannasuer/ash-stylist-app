...
const CSS = `
  ...
  @keyframes vPulse {
    0%   { filter:brightness(1) drop-shadow(0 0 0px #C41E1E); transform:scale(1) }
    35%  { filter:brightness(1.6) drop-shadow(0 0 12px #C41E1E); transform:scale(1.12) }
    70%  { filter:brightness(0.8) drop-shadow(0 0 2px #C41E1E); transform:scale(0.94) }
    100% { filter:brightness(1) drop-shadow(0 0 0px #C41E1E); transform:scale(1) }
  }
  @keyframes vFloat { 0%,100% { opacity:0.4 } 50% { opacity:1 } }

  .a-up   { animation: msgUp  .28s cubic-bezier(.16,1,.3,1) forwards }
  .a-fade { animation: fadeIn .2s ease forwards }
  .a-sl   { animation: slideL .32s cubic-bezier(.16,1,.3,1) forwards }
  .a-sr   { animation: slideR .32s cubic-bezier(.16,1,.3,1) forwards }
  .a-cur  { display:inline-block; width:2px; height:.85em; background:#C41E1E; margin-left:1px; vertical-align:text-bottom; animation:blink 1s step-end infinite }
  .a-vp   { animation:vPulse 1.8s ease-in-out infinite; transform-origin:center }
  .a-vt   { animation:vFloat 1.8s ease-in-out infinite }

  .a-sc::-webkit-scrollbar       { width:4px }
  .a-sc::-webkit-scrollbar-track { background:transparent }
  .a-sc::-webkit-scrollbar-thumb { border-radius:4px; background:#D5D0C8 }
  ...
  .a-pc:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(0,0,0,0.10) }
`;
    ...
    if (l.startsWith('**') && l.endsWith('**') && !l.slice(2, -2).includes('**')) {
      ...
      out.push(
        <ul key={'u' + i} style={{ listStyle: 'none', padding: 0, margin: '5px 0' }}>
          {its.map((it, j) => (
            <li key={j} style={{ display: 'flex', gap: 7, margin: '4px 0', fontFamily: sans, fontSize: 14, lineHeight: 1.7, color: C.txt }}>
...
function Av({ isAsh, pulse }) {
  if (isAsh) return (
    <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'transparent', border: `1px solid ${C.bdr}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <img src={ASH_LOGO} className={pulse ? 'a-vp' : ''} style={{ width: 26, height: 26, objectFit: 'contain' }} alt="" />
    </div>
  );
  return (
...
function Thinking() {
  return (
    <div className="a-up" style={{ display: 'flex', gap: 12, marginBottom: 22, maxWidth: 760, marginLeft: 'auto', marginRight: 'auto' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.surf, border: `1px solid ${C.bdr}`, borderRadius: '4px 16px 16px 16px', padding: '12px 18px', boxShadow: C.shd }}>
          <img src={ASH_LOGO} className="a-vp" style={{ width: 20, height: 20, objectFit: 'contain', flexShrink: 0 }} alt="" />
          <span className="a-vt" style={{ fontFamily: serif, fontStyle: 'italic', fontSize: 15, color: C.fnt }}>Styling your look…</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <a href={p.link} target="_blank" rel="noreferrer" className="a-pc"
      ...
      <div style={{ padding: '10px 12px 12px' }}>
        <div style={{ fontFamily: serif, fontWeight: 600, fontSize: 15, color: C.txt, marginBottom: 2 }}>{p.name}</div>
...
function Lookbook({ look, onClose }) {
  ...
  return (
    <>
      <div className="a-fade" onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.28)', zIndex: 200, backdropFilter: 'blur(3px)' }} />
      <div className="a-sr a-sc" style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 296, background: C.sb, borderLeft: `1px solid ${C.bdr}`, zIndex: 201, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
...
function Sidebar({ convos, active, onSel, onNew, onDel, onRen, onClose }) {
  ...
  return (
    <>
      <div className="a-fade" onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.18)', zIndex: 98 }} />
      <div className="a-sl a-sc" style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 252, background: C.sb, borderRight: `1px solid ${C.bdr}`, zIndex: 99, display: 'flex', flexDirection: 'column', boxShadow: '4px 0 20px rgba(0,0,0,0.08)' }}>
        <div style={{ padding: '13px 14px', borderBottom: `1px solid ${C.bdr}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <img src={ASH_LOGO} style={{ width: 20, height: 20, objectFit: 'contain' }} alt="" />
            <span style={{ fontFamily: serif, fontWeight: 600, fontSize: 14, color: C.acc, letterSpacing: '.15em' }}>ASH ×</span>
          </div>
          <div style={{ display: 'flex', gap: 5 }}>
            <button className="a-ib" onClick={onNew} style={{ background: C.accBg, border: `1px solid ${C.acc}40`, borderRadius: 7, width: 26, height: 26, color: C.acc }}><Plus size={13} /></button>
            <button className="a-ib" onClick={onClose} style={{ width: 26, height: 26, borderRadius: 7, color: C.fnt }}><X size={13} /></button>
          </div>
        </div>
        <div style={{ padding: '5px 8px 3px', fontFamily: sans, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.14em', color: C.fnt }}>Conversations</div>
        <div className="a-sc" style={{ flex: 1, overflowY: 'auto', padding: '2px 6px 8px' }}>
          ...
          {convos.map(c => (
            <div key={c.id} className="a-ci a-hov" onClick={() => { onSel(c.id); onClose(); }}
              ...
              {eid === c.id
                ? <input value={etxt} onChange={e => setEtxt(e.target.value)}
                    ...
                    style={{ width: '100%', background: 'none', border: `1px solid ${C.acc}`, borderRadius: 4, padding: '2px 5px', fontFamily: sans, fontSize: 13, color: C.txt, outline: 'none' }} />
                : <>
                    ...
                    <div className="a-hv" style={{ display: 'flex', gap: 2, marginTop: 3 }}>
                      <button onClick={e => { e.stopPropagation(); setEid(c.id); setEtxt(c.title); }} className="a-ib" style={{ width: 18, height: 18, borderRadius: 3, color: C.fnt }}><Edit3 size={9} /></button>
                      <button onClick={e => { e.stopPropagation(); onDel(c.id); }} className="a-ib" style={{ width: 18, height: 18, borderRadius: 3, color: C.fnt }}><Trash2 size={9} /></button>
                    </div>
                  </>
              }
            </div>
...
function Message({ msg, onRegen, onLook, isLast, streamText, isStreaming }) {
  ...
  return (
    <div className="a-up a-hov" style={{ display: 'flex', gap: 12, marginBottom: 24, maxWidth: 760, marginLeft: 'auto', marginRight: 'auto', flexDirection: isAsh ? 'row' : 'row-reverse' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div>
          {isAsh
            ? <><MD text={content} />{isStreaming && isLast && <span className="a-cur" />}</>
            : <p style={{ margin: 0, fontFamily: sans, fontSize: 14, lineHeight: 1.78, color: C.txt }}>{content}</p>
          }
        </div>
        ...
        {!isStreaming && (
          <div className="a-hv" style={{ display: 'flex', gap: 3, marginTop: 6 }}>
            <button className="a-ib" onClick={copy} style={{ width: 26, height: 26, borderRadius: 6, color: copied ? '#22c55e' : C.fnt }}>{copied ? <Check size={12} /> : <Copy size={12} />}</button>
            {isAsh && isLast && <button className="a-ib" onClick={onRegen} style={{ width: 26, height: 26, borderRadius: 6, color: C.fnt }}><RefreshCw size={12} /></button>}
          </div>
        )}
      </div>
    </div>
    ...
    <div style={{ position: 'fixed', top: 64, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: C.bg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 14px', borderBottom: `1px solid ${C.bdr}`, background: C.surf, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <button className="a-ib" onClick={() => setSbOpen(true)} title="Chat history"
            style={{ width: 30, height: 30, borderRadius: 8, color: C.mid, background: C.bg, border: `1px solid ${C.bdr}` }}>
            <Menu size={14} />
          </button>
          <button className="a-ib" onClick={newChat} title="New conversation"
        ...
        <span style={{ fontFamily: sans, fontSize: 11, color: C.fnt, letterSpacing: '.06em' }}>
          {profile
            ? <span style={{ color: C.acc, fontWeight: 600 }}>✦ {profile.name?.split(' ')[0]} — FIT ID Active</span>
            : 'Ask Ash anything'
          }
        </span>
      ...
      {/* Messages — flex:1 + minHeight:0 keeps input pinned at bottom */}
      <div className="a-sc" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '24px 18px 8px' }}>
        {isWelcome && (
          <div className="a-fade" style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <div style={{ fontFamily: sans, fontSize: 14, color: C.fnt, lineHeight: 1.7, maxWidth: 340, margin: '0 auto' }}>
              Ask Ash anything — outfits, occasions, body types, trends.<br />She knows exactly what works.
            </div>
          </div>
        )}
        {display.map((m, i) =>
          m.isPend
            ? <Thinking key={m.id} />
            : <Message key={m.id} msg={m} onRegen={regen} onLook={setLookbook} isLast={i === lastAshI} streamText={stxt} isStreaming={m.id === streamId && srun} />
        )}
        <div ref={bottomRef} />
      </div>
        ...
        <div style={{ padding: '0 18px 12px', display: 'flex', flexWrap: 'wrap', gap: 7, maxWidth: 760, margin: '0 auto', width: '100%', boxSizing: 'border-box', flexShrink: 0 }}>
          {QUICK.map(q => (
            <button key={q} onClick={() => send(q)} className="a-ch"
              style={{ background: C.surf, border: `1px solid ${C.bdr}`, borderRadius: 24, padding: '7px 15px', fontFamily: sans, fontSize: 12, fontWeight: 500, color: C.mid, boxShadow: C.shd, cursor: 'pointer' }}>
              {q}
            </button>
          ))}
        </div>
      ...
      {imgPrev && (
        <div style={{ padding: '0 18px 7px', maxWidth: 760, margin: '0 auto', width: '100%', boxSizing: 'border-box', flexShrink: 0 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: C.surf, border: `1px solid ${C.bdr}`, borderRadius: 10, padding: 7 }}>
            ...
            <button className="a-ib" onClick={() => setImgPrev(null)} style={{ color: C.fnt, width: 18, height: 18 }}><X size={12} /></button>
          </div>
        </div>
      )}

      {/* Input — flexShrink:0 keeps it always visible at the bottom */}
      <div style={{ padding: '9px 16px 13px', borderTop: `1px solid ${C.bdr}`, background: C.surf, flexShrink: 0 }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 7, alignItems: 'center', background: C.bg, border: `1.5px solid ${C.bdr}`, borderRadius: 20, padding: '9px 10px 9px 14px', boxShadow: C.shd }}>
            ...
            <button className="a-ib" onClick={() => fileRef.current?.click()} style={{ width: 28, height: 28, borderRadius: 7, color: C.fnt, flexShrink: 0 }}><Paperclip size={14} /></button>
            <input
              value={input}
            ...
            />
            {loading
              ? <button className="a-ib" onClick={stop} style={{ width: 34, height: 34, borderRadius: 10, background: C.accBg, color: C.acc, flexShrink: 0 }}><X size={14} /></button>
